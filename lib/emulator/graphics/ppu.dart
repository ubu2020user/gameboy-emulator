...
class PPU {
  ...
  List<int> spritesDrawnPerLine = List.filled(PPU.LCD_HEIGHT, 0);
  ...
  PPU(this.cpu) {
    // this.buffer.fillRange(0, PPU.LCD_WIDTH * PPU.LCD_HEIGHT, 0);
    // this.current.fillRange(0, PPU.LCD_WIDTH * PPU.LCD_HEIGHT, 0);
    // this.gbcSpritePaletteMemory.fillRange(0, 0x40, 0);
    // this.spritesDrawnPerLine.fillRange(0, PPU.LCD_HEIGHT, 0);
  ...
  void reset() {
    ...
    this.bgPalettes = List.filled(8, GBPalette(cpu, [0xFF, 0xFF, 0xFF, 0xFF], MemoryRegisters.BGP)); //(8);
    this.spritePalettes = List.filled(8, GBPalette(cpu, [0xFF, 0xFF, 0xFF, 0xFF], MemoryRegisters.BGP)); //(8);
    this.gbcBackgroundPaletteMemory = List.filled(0x40, 0); //(0x40);

    this.buffer = List.filled(
        PPU.LCD_WIDTH * PPU.LCD_HEIGHT, 0); //(PPU.LCD_WIDTH * PPU.LCD_HEIGHT);

    this.current = List.filled(
        PPU.LCD_WIDTH * PPU.LCD_HEIGHT, 0); //(PPU.LCD_WIDTH * PPU.LCD_HEIGHT);

    this.gbcSpritePaletteMemory = List.filled(0x40, 0); //(0x40);

    this.spritesDrawnPerLine = List.filled(PPU.LCD_HEIGHT, 0);

    this.spritesDrawnPerLine =
        List.filled(PPU.LCD_HEIGHT, 0); //(PPU.LCD_HEIGHT);

    if (this.cpu.cartridge.gameboyType == GameboyType.COLOR) {
      this
          .gbcBackgroundPaletteMemory
          .fillRange(0, this.gbcBackgroundPaletteMemory.length, 0x1f);

      for (int i = 0; i < this.spritePalettes.length; i++) {
        List<int> colors = List.filled(4, 0); //(4);
        this.spritePalettes[i] = GBCPalette(colors);
      }

      for (int i = 0; i < this.bgPalettes.length; i++) {
        List<int> colors = List.filled(4, 0); //(4);
        this.bgPalettes[i] = GBCPalette(colors);
      }
      ...
      PaletteColors colors =
          PaletteColors.getByHash(this.cpu.cartridge.checksum);

      this.bgPalettes[0] = GBPalette(this.cpu, colors.bg, MemoryRegisters.BGP);
      this.spritePalettes[0] =
          GBPalette(this.cpu, colors.obj0, MemoryRegisters.OBP0);
      this.spritePalettes[1] =
          GBPalette(this.cpu, colors.obj1, MemoryRegisters.OBP1);
    }
  }
  ...
  void setBackgroundPalette(int reg, int data) {
    ...
    int palette = reg >> 3;
    updatePalette(this.gbcBackgroundPaletteMemory, this.bgPalettes[palette],
        palette, (reg >> 1) & 0x3);
  }
  ...
  void setSpritePalette(int reg, int data) {
    ...
    int palette = reg >> 3;
    updatePalette(this.gbcSpritePaletteMemory, this.spritePalettes[palette],
        palette, (reg >> 1) & 0x3);
  }
  ...
  void tick(int cycles) {
    ...
    if (this.lcdCycles >= 456) {
      ...
      this
          .cpu
          .mmu
          .writeRegisterByte(MemoryRegisters.LY, (((ly + 1) % 154) & 0xff));
      ...
      this.cpu.mmu.writeRegisterByte(MemoryRegisters.LCD_STAT,
          this.cpu.mmu.readRegisterByte(MemoryRegisters.LCD_STAT) & ~0x03);
      ...
      this.cpu.mmu.writeRegisterByte(MemoryRegisters.LCD_STAT,
          this.cpu.mmu.readRegisterByte(MemoryRegisters.LCD_STAT) | mode);
      ...
      if (displayEnabled && !isVBlank) {
        ...
        if ((lcdStat &
          ...
          if (lyc == ly) {
            ...
            this.cpu.mmu.writeRegisterByte(
                MemoryRegisters.LCD_STAT,
                this.cpu.mmu.readRegisterByte(MemoryRegisters.LCD_STAT) |
  ...
  void draw(int scanline) {
    ...
    if (this.windowEnabled() &&
        scanline >= this.getWindowPosY() &&
        this.getWindowPosX() < LCD_WIDTH &&
        this.getWindowPosY() >= 0) {
  ...
  void drawBackgroundTiles(List<int> data, int scanline) {
    ...
    for (int x = 0; x < 21; x++) {
      ...
      int tile = tileDataOffset == 0
          ? (this.cpu.mmu.readVRAM(addressBase) & 0xFF)
          : (this.cpu.mmu.readVRAM(addressBase) + 256);
      ...
      if (this.cpu.cartridge.gameboyType == GameboyType.COLOR) {
        int attributes =
            this.cpu.mmu.readVRAM(MemoryAddresses.VRAM_PAGESIZE + addressBase);
  ...
  void drawTile(
    ...
    for (int px = 0; px < 8; px++) {
      ...
      int paletteUpper =
          (((this.cpu.mmu.readVRAM(address + 1) & (0x80 >> logicalX)) >>
                  (7 - logicalX)) <<
              1);
      // lower bit of the color number
      int paletteLower =
          ((this.cpu.mmu.readVRAM(address) & (0x80 >> logicalX)) >>
              (7 - logicalX));
  ...
  void drawSprites(List<int> data, int scanline) {
    ...
    for (int i = 0;
        i < this.cpu.mmu.oam.length && this.spritesDrawnPerLine[scanline] < 10;
        i += 4) {
      ...
      // Palette selection
      Palette pal = this.spritePalettes[
      ...
      if (tall) {
        ...
        if (y - 16 <= scanline && scanline < y - 8) {
          this.drawTile(pal, data, x - 8, y - 16, hi, scanline, flipX, flipY,
              vrambank, priority, true);
          this.spritesDrawnPerLine[scanline]++;
        }

        if (y - 8 <= scanline && scanline < y) {
          this.drawTile(pal, data, x - 8, y - 8, lo, scanline, flipX, flipY,
              vrambank, priority, true);
          this.spritesDrawnPerLine[scanline]++;
        }
      } else {
        this.drawTile(pal, data, x - 8, y - 16, tile, scanline, flipX, flipY,
            vrambank, priority, true);
        this.spritesDrawnPerLine[scanline]++;
      }
    }
  ...
  bool displayEnabled() {
    return (this.cpu.mmu.readRegisterByte(MemoryRegisters.LCDC) &
  ...
  int getWindowTileMapOffset() {
    if ((this.cpu.mmu.readRegisterByte(MemoryRegisters.LCDC) &
