...
class Memory {
  ...
  void reset() {
    ...
    this.registers = Uint8List(0x100);
    this.registers.fillRange(0, this.registers.length, 0);

    this.oam = Uint8List(0xA0);
    this.oam.fillRange(0, this.oam.length, 0);

    this.wram = Uint8List(Memory.WRAM_PAGESIZE *
        (this.cpu.cartridge.gameboyType == GameboyType.COLOR ? 8 : 2));
    this.wram.fillRange(0, this.wram.length, 0);

    this.vram = Uint8List(Memory.VRAM_PAGESIZE *
        (this.cpu.cartridge.gameboyType == GameboyType.COLOR ? 2 : 1));
  ...
  void writeByte(int address, int value) {
    ...
    if (value > 0xFF) {
      throw Exception('Tring to write ' +
    ...
    // VRAM
      ...
      this.vram[this.vramPageStart +
          address -
          MemoryAddresses.VIDEO_RAM_START] = value;
    }
    // Cartridge RAM
  ...
  int readByte(int address) {
    ...
    if (address >= MemoryAddresses.CARTRIDGE_ROM_SWITCHABLE_START &&
      ...
      return this.cpu.cartridge.data[this.romPageStart +
          address -
    ...
    // VRAM
        ...
        address < MemoryAddresses.VIDEO_RAM_END) {
      return this
          .vram[this.vramPageStart + address - MemoryAddresses.VIDEO_RAM_START];
    }
    // Cartridge RAM
    else if (address >= MemoryAddresses.SWITCHABLE_RAM_START &&
        address < MemoryAddresses.SWITCHABLE_RAM_END) {
    ...
    // RAM A
      ...
      return this.wram[address - MemoryAddresses.RAM_A_START];
        ...
        address < MemoryAddresses.RAM_A_END) {
      return this.wram[this.wramPageStart +
          address -
          MemoryAddresses.RAM_A_SWITCHABLE_START];
    }
    // RAM echo
  ...
  void writeIO(int address, int value) {
    ...
    if (address > 0xFF) {
      throw Exception('Tring to write register ' +
          value.toRadixString(16) +
          ' into ' +
          address.toRadixString(16) +
          ' (>0xFF).');
    ...
    // Background Palette Data
      ...
      if (this.cpu.cartridge.gameboyType == GameboyType.COLOR) {
        ...
        if ((index & 0x80) != 0) {
          ...
          this.registers[MemoryRegisters.BACKGROUND_PALETTE_INDEX] =
    ...
    // Sprite Palette Data
      ...
      if (this.cpu.cartridge.gameboyType == GameboyType.COLOR) {
        ...
        if ((index & 0x80) != 0) {
          currentRegister++;
          currentRegister %= 0x40;
          this.registers[MemoryRegisters.SPRITE_PALETTE_INDEX] =
              (0x80 | currentRegister) & 0xFF;
        }
      }
    }
    // Start H-DMA transfer
      ...
      if (this.cpu.cartridge.gameboyType != GameboyType.CLASSIC) {
        ...
        int source = ((this.registers[0x51] & 0xff) << 8) |
            (this.registers[0x52] & 0xF0);
        int destination = ((this.registers[0x53] & 0x1f) << 8) |
            (this.registers[0x54] & 0xF0);

        // H-Blank DMA
        if ((value & 0x80) != 0) {
          this.dma = DMA(this, source, destination, length);
          ...
          for (int i = 0; i < length; i++) {
            this.vram[this.vramPageStart + destination + i] =
    ...
    // OAM DMA transfer
      ...
      if (((this.registers[MemoryRegisters.TAC] ^ value) & 0x03) != 0) {
        ...
        this.registers[MemoryRegisters.TIMA] =
            this.registers[MemoryRegisters.TMA];
      }
  ...
  int readIO(int address) {
    if (address > 0xFF) {
      throw Exception('Tring to read register from ' +
          address.toRadixString(16) +
          ' (>0xFF).');
    }
    ...
    if (address == MemoryRegisters.DOUBLE_SPEED) {
      ...
      if (reg & 0x10 == 0) {
        if (this.cpu.buttons[Gamepad.RIGHT]) {
          reg &= ~0x1;
        }
        if (this.cpu.buttons[Gamepad.LEFT]) {
          reg &= ~0x2;
        }
        if (this.cpu.buttons[Gamepad.UP]) {
          reg &= ~0x4;
        }
        if (this.cpu.buttons[Gamepad.DOWN]) {
          reg &= ~0x8;
        }
      }

      if (reg & 0x20 == 0) {
        if (this.cpu.buttons[Gamepad.A]) {
          reg &= ~0x1;
        }
        if (this.cpu.buttons[Gamepad.B]) {
          reg &= ~0x2;
        }
        if (this.cpu.buttons[Gamepad.SELECT]) {
          reg &= ~0x4;
        }
        if (this.cpu.buttons[Gamepad.START]) {
          reg &= ~0x8;
        }
      }
